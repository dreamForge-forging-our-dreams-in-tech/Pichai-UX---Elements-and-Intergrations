...
class GithubMde extends HTMLElement {
    ...
    async connectedCallback() {
        ...
        let labels, repos_select, title_input;
        ...
        if (this.getAttribute('mode') == 'issue' || !this.hasAttribute('mode')) { // if it is a issue load all items required for issues.

            const info = document.createElement('div');
            info.style.width = 'calc(100% - 16px)';
            info.classList.add('dialog-content', 'row');

            labels = document.createElement('github-labels');
            ...
            repos_select = document.createElement('select');
