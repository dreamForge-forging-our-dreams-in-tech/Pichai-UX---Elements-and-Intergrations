...
class GithubIntergration {
    constructor(repUrl, orgName = 'dreamForge-forging-our-dreams-in-tech', repoName = 'The-Magic-Garden') {
        ...
        if(!this.repUrl) {
            this.repUrl = `https://api.github.com/repos/${this.orgName}/${this.repoName}/`;
        }
    }
    ...
    async get_issue(issueNumber, repoUrl = this.repoUrl + 'issues?state=all&per_page=100') {
    ...
    async get_comments(issue_number, repoUrl = this.repoUrl + 'issues') {
    ...
    async loadGitHubIssues(url = this.repoUrl + 'issues?state=all&per_page=100') {
    ...
    async listReposWithLogos(orgName = this.orgName) { // retrieves a lists of all github repos for a org and displays them in the provided container.
        try {
    ...
    async list_labels(orgName = this.orgName, repo = this.repoName) { // retrieves a lists of all github repos for a org and displays them in the provided container.
        try {
    ...
    async create_comment(body, issue_number, owner, repoUrl = this.repoUrl + 'issues') {
        const issuePayload = {
            issueNumber: issue_number,
            body: body,
            owner: owner,
        ...
        async create_issue(orgname = this.orgName, repo = this.repoName, title, body, labels) {
        const issuePayload = {
            owner: orgname,
            repo: repo,
            title: title,
